...
class WC_Product {
	...
	function get_price_html_from_to( $from, $to ) {
		return '<ins style="color:Blue;">' . ( ( is_numeric( $to ) ) ? woocommerce_price( $to ) : $to ) . '</ins><br/><del style="font-size:medium;float:right;">' . ( ( is_numeric( $from ) ) ? woocommerce_price( $from ) : $from ) . '</del>';
	}
