...
class Etheme_Recent_Posts_Widget extends WP_Widget {
    ...
    function Etheme_Recent_Posts_Widget() {
        ...
        $this->WP_Widget('etheme-recent-posts', '8theme - '.__('Recent Posts', ETHEME_DOMAIN), $widget_ops);
                ...
                <?php  while ($r->have_posts()) : $r->the_post(); ?>
                    <li>
                        <?php 
                            ...
                            if($url && $url != ''):
                        ?>
                            <a class="postimg" href="<?php the_permalink() ?>" title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>"><img src="<?php echo etheme_get_image(false,60,60); ?>" /></a>
                        ...
                        <span class="recent-posts-2-author"><?php the_author(); ?> <?php the_date('F j, Y','/ '); ?></span>
                        <div class="clear"></div>
                    </li>
                <?php endwhile; ?>
            </ul>
...
<?php
	...
	function widget( $args, $instance ) {
		...
		if ( isset( $cache[ $args['widget_id'] ] ) ) {
 		...
 		$output = '';
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Recent Comments' , ETHEME_DOMAIN) : $instance['title'], $instance, $this->id_base );
		...
		if ( $comments ) {
			foreach ( (array) $comments as $comment) {
				$output .=  '<li class="recentcomments">' . sprintf(_x('<span class="comment_author">%1$s</span> <br> %2$s', 'widgets'), get_comment_author_link(), '<span class="comment_link"><a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . get_the_title($comment->comment_post_ID) . '</a></span>') . '</li>';
			}
 		}
		$output .= '</ul>';
		$output .= $after_widget;
