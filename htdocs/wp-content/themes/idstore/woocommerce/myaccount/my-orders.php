	...
	<table class=" my_account_orders table">
		...
		<thead>
			<tr>
				...
				<th class="order-shipto"><span class="nobr"><?php _e('Ship to', ETHEME_DOMAIN); ?></span></th>
				<th class="order-total"><span class="nobr"><?php _e('Total', ETHEME_DOMAIN); ?></span></th>
				<th class="order-status" colspan="2"><span class="nobr"><?php _e('Status', ETHEME_DOMAIN); ?></span></th>
			</tr>
		</thead>
		...
		<tbody><?php
			foreach ($customer_orders as $customer_order) :
				...
				?><tr class="order">
					<td class="order-number" width="19%">
					...
					<td class="order-status">
						...
						<?php if (in_array($order->status, array('pending', 'failed'))) : ?>
							<a href="<?php echo esc_url( $order->get_cancel_order_url() ); ?>" class="cancel" title="<?php _e('Click to cancel this order', ETHEME_DOMAIN); ?>">(<?php _e('Cancel', ETHEME_DOMAIN); ?>)</a>
						<?php endif; ?>
					</td>
					<td class="order-actions">
												
						<?php if (in_array($order->status, array('pending', 'failed'))) : ?>
							<a href="<?php echo esc_url( $order->get_checkout_payment_url() ); ?>" class="button fl-l pay"><span><?php _e('Pay', ETHEME_DOMAIN); ?></span></a>
						<?php endif; ?>
						
						<a href="<?php echo esc_url( add_query_arg('order', $order->id, get_permalink(woocommerce_get_page_id('view_order'))) ); ?>" class="button"><span><?php _e('View', ETHEME_DOMAIN); ?></span></a>
...
?>
	<p><?php _e('You have no recent orders.', ETHEME_DOMAIN); ?></p>
<?php
endif;
?>
