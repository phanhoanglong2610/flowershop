...
$tiny_contact_form = new toeTinyContactForm();

class toeTinyContactForm
{
...
function showForm( $params = '' )
{
	...
	$captcha = new toeTinyContactFormCaptcha( rand(1000000000, 9999999999) );
	...
	if ( empty($result) || (!empty($result) && !$this->o['hideform']) )
	{
		...
		$form .= '
			...
			<div id="footer_form">
			<input name="tcf_name'.$n.'" id="tcf_name'.$n.'" value="" type="text" class="tcf_input"/>
			<input name="tcf_sendit'.$n.'" id="tcf_sendit'.$n.'" value="1" class="tcf_input" />
			<input name="tcf_sender'.$n.'" id="tcf_sender'.$n.'" size="30" type="text"
                            value="'.$tcf_sender.'" class="tcf_field" placeholder="'.__('Nhập tên của bạn', 'ready_ecommerce').'" />
			<input name="tcf_email'.$n.'" id="tcf_email'.$n.'" size="30" type="text"
                            value="'.$tcf_email.'" class="tcf_field" placeholder="'.__('Nhập email của bạn', 'ready_ecommerce').'" />';
		// additional fields
		for ( $x = 1; $x <=5; $x++ )
		{
			$i = 'tcf_field_'.$x.$n;
			...
			$f = $this->o['field_'.$x];
			if ( !empty($f) )
				$form .= '<input name="'.$i.'" id="'.$i.'" size="30" type="text" value="'.$tcf_f.'" class="tcf_field" placeholder="'.$f.'"/>';
		}
		$form .= '<input name="tcf_subject'.$n.'" id="tcf_subject'.$n.'" size="30" 
                                 value="'.$tcf_subject.'" class="tcf_field" type="hidden" placeholder="'.__('enter subject','ready_ecommerce').'" />
			<textarea name="tcf_msg'.$n.'" id="tcf_msg'.$n.'" class="tcf_textarea" 
                                cols="50" rows="5" placeholder="'.__('','ready_ecommerce').'">'.$tcf_msg.'</textarea>
			';
		if ( $this->o['captcha'] )
		...
		$title = (!empty($this->o['submit'])) ? 'value="'.$this->o['submit'].'"' : 'value="'.__('Gửi','ready_ecommerce').'"';
		$form .= '	
...
{	
	...
	<div id="poststuff" class="wrap">
		...
		<form action="admin.php?page=tiny-contact-form" method="post">
	...
	<div class="postbox">
		...
		<div class="inside">
			...
			<p><a href="<?php echo get_bloginfo('template_directory') ?>/tiny-contact-form/readme.txt?KeepThis=true&amp;TB_iframe=true" title="Tiny Contact Form - Readme.txt" class="thickbox"><strong>Readme.txt</strong></a></p>
		</div>
	</div>
...
function addOptionsPage()
{
	...
	add_submenu_page('theme_settings', 'Contact Form Settings', $menutitle, 'edit_theme_options', 'tiny-contact-form', array( &$this, 'optionsPage'));
}
...
//    extra felder
	...
	if ( $o['captcha'] && !toeTinyContactFormCaptcha::isCaptchaOk() )
...
function pluginActions($links, $file)
{
	...
	{
		$link = '<a href="admin.php?page=tiny-contact-form">'.__('Settings').'</a>';
...
{
	register_widget('toeTinyContactForm_Widget');
}
 ...
 * Captcha class
 */
class toeTinyContactFormCaptcha
{
 ...
 */
function toeTinyContactFormCaptcha( $seed )
{
...
function isCaptchaOk()
{
	...
	if ($_POST[base64_encode(strrev('current_time'))] && $_POST[base64_encode(strrev('captcha'))])
	{
		...
		if ((time() - strrev(base64_decode($_POST[base64_encode(strrev('current_time'))]))) > 1800)
			$ok = false;
		// check answer
		$valid = new toeTinyContactFormCaptcha(strrev(base64_decode($_POST[base64_encode(strrev('captcha'))])));
...
class toeTinyContactForm_Widget extends WP_Widget
{
	 ...
	 * constructor
	 */	 
	function toeTinyContactForm_Widget() {
