...
function build_pages($post_parent = 0) {	
	...
	foreach($posts as $p) {								
		...
		if(get_the_post_thumbnail($p->ID, 'reorder-thumb') !== FALSE) {	
		echo '<div class="thumb">' . get_the_post_thumbnail($p->ID, 'reorder-thumb') . '</div>';	
		}	
...
function reorder_head(){	
	echo '<link type="text/css" rel="stylesheet" href="' . get_template_directory_uri(). '/lib/reorder/reorder.css" />' . "\n";
...
function reorder_script() {	
	wp_enqueue_script('reorderScript', get_template_directory_uri(). '/lib/reorder/interface/iutil.js');
	wp_enqueue_script('reorderScript1', get_template_directory_uri(). '/lib/reorder/interface/idrag-modified.js');
	wp_enqueue_script('reorderScript2', get_template_directory_uri(). '/lib/reorder/interface/idrop.js');
	wp_enqueue_script('reorderScript3', get_template_directory_uri(). '/lib/reorder/interface/isortables.js');		
	wp_enqueue_script('reorderScript4', get_template_directory_uri(). '/lib/reorder/inestedsortable.js');			
	wp_enqueue_script('reorderScript5', get_template_directory_uri(). '/lib/reorder/reorder.js.php');		
}
