...
add_action( 'wp_head', 'ar_wp_head', 10 );

if ( ! function_exists( 'ar_wp_head' ) ) {
	function ar_wp_head() {
	// Output shortcodes stylesheet
		if ( function_exists( 'ar_shortcode_stylesheet' ) )
			ar_shortcode_stylesheet();
...
if ( ! function_exists( 'ar_shortcode_stylesheet' )) {
	function ar_shortcode_stylesheet() {
		echo "\n" . "<!-- Shortcodes CSS -->\n";
		echo '<link href="'. get_template_directory_uri() . '/shortcode/css/shortcodes.css" rel="stylesheet" type="text/css" />'."\n";
	} // End ar_shortcode_stylesheet()
}
...
add_action( 'wp_print_scripts', 'ar_register_shortcode_js', 10 );

function ar_register_shortcode_js () {
	wp_register_script( 'ar-shortcodes', get_template_directory_uri() . '/shortcode/js/shortcodes.js', array( 'jquery', 'jquery-ui-tabs' ), '5.0.0' );
} // End ar_register_shortcode_js()

add_action( 'wp_footer', 'ar_enqueue_shortcode_js', 10 );

function ar_enqueue_shortcode_js () {
	if ( ! is_admin() ) {
		wp_enqueue_script( 'ar-shortcodes' );
		
		global $wp_scripts;
		$wp_scripts->to_do = array( 'ar-shortcodes' );
		
		wp_print_scripts();
	}
} // End ar_enqueue_shortcode_js()
...
function _dep_woo_shortcode_related_posts( $atts ) { trigger_error( sprintf( __( 'Please upgrade WordPress', 'selftitled' ), '_dep_woo_shortcode_related_posts', '5.4' ) ); }
...
function woo_shortcode_twitter($atts, $content = null) {
	...
	$output = '<div class="woo-sc-twitter ' . esc_attr( $float ) . '"><a href="' . esc_url( 'http://twitter.com/share' ) . '" class="twitter-share-button"'. $output .'>' . __( 'Tweet', 'selftitled' ) . '</a><script type="text/javascript" src="' . esc_url ( 'http://platform.twitter.com/widgets.js' ) . '"></script></div>';
	return $output;
...
function woo_shortcode_toggle ( $atts, $content = null ) {
		...
		$defaults = array(
							'title_open' => __( 'Hide the Content', 'selftitled' ),
							'title_closed' => __( 'Show the Content', 'selftitled' ),
							'hide' => 'yes',
							...
							'read_more_text' => __( 'Read More', 'selftitled' ),
							'read_less_text' => __( 'Read Less', 'selftitled' )
						);
...
function woo_shortcode_contactform ( $atts, $content = null ) {
		$defaults = array(
						...
						'subject' => __( 'Message via the contact form', 'selftitled' ),
						'button_text' => apply_filters( 'woo_contact_form_button_text', __( 'Submit', 'selftitled' ) ), 
		...
		if ( ( count( $_POST ) > 2 ) && isset( $_POST['submitted'] ) ) {
			...
			$error_responses = array(
									'contactName' => __( 'Please enter your name', 'selftitled' ),
									'contactEmail' => __( 'Please enter your email address (and please make sure it\'s valid)', 'selftitled' ),
									'contactMessage' => __( 'Please enter your message', 'selftitled' )
									);
			...
			if ( count( $error_messages ) ) {} else {
				...
				// Send the e-mail.
				$headers = __( 'From: ', 'selftitled') . $default_fields['contactName'] . ' <' . $default_fields['contactEmail'] . '>' . "\r\n" . __( 'Reply-To: ', 'selftitled' ) . $default_fields['contactEmail'];
		...
		if( isset( $emailSent ) && $emailSent == true ) {
			$html .= do_shortcode( '[box type="tick"]' . __( 'Your email was successfully sent.', 'selftitled' ) . '[/box]' );
			$html .= '<span class="has_sent hide"></span>' . "\n";
		}

		// Error messages
		if( count( $error_messages ) ) {
			$html .= do_shortcode( '[box type="alert"]' . __( 'There were one or more errors while submitting the form.', 'selftitled' ) . '[/box]' );
		}

        // No e-mail address supplied.
        if( $email == '' ) {
			$html .= do_shortcode( '[box type="alert"]' . __( 'E-mail has not been setup properly. Please add your contact e-mail!', 'selftitled' ) . '[/box]' );
		}
		...
		if ( $email == '' ) {} else {
			...
			if ( $show_default_fields != 'no' ) {
				...
				$html .= '<p><label for="contactName">' . __( 'Name', 'selftitled' ) . '</label>' . "\n";
				...
				$html .= '<p><label for="contactEmail">' . __( 'Email', 'selftitled' ) . '</label>' . "\n";
				...
				$html .= '<p class="textarea"><label for="contactMessage">' . __( 'Message', 'selftitled' ) . '</label>' . "\n";
			...
			$html .= '<p class="inline"><input type="checkbox" name="sendCopy" id="sendCopy" value="true"' . esc_attr( $sendCopy ) . ' /><label for="sendCopy">' . __( 'Send a copy of this email to yourself', 'selftitled' ) . '</label></p>' . "\n";
			...
			$html .= '<p class="screenReader"><label for="checking" class="screenReader">' . __('If you want to submit this form, do not enter anything in this field', 'selftitled') . '</label><input type="text" name="checking" id="checking" class="screenReader" value="' . esc_attr( $checking ) . '" /></p>' . "\n";
...
function woo_shortcode_socialicon ( $atts, $content = null ) {
	...
	if ( $profile_type != '' ) {
		...
		if ( $title ) { $_alt_text = $title; } else { $_alt_text = ucwords( $_profile_to_display ); $_alt_text = sprintf( __( 'My %s Profile', 'selftitled' ), $_alt_text ); } // End IF Statement
	...
	if ( $url == 'feed' ) {
		...
		if ( $title ) { $_alt_text = $title; } else { $_alt_text = __( 'Subscribe to our RSS feed', 'selftitled' ); } // End IF Statement
		...
		foreach ( $_supported_profiles as $k => $v ) {
			...
			if ( $host == $v ) {
				...
				if ( $title ) { $_alt_text = $title; } else { $_alt_text = ucwords( $matches[1] ); $_alt_text = sprintf( __( 'My %s Profile', 'selftitled' ), $_alt_text ); } // End IF Statement
...
function woo_shortcode_twitter_follow ( $atts, $content = null ) {
	...
	$output .= '<a href="'. esc_url( 'https://twitter.com/' . $username ) . '" class="twitter-follow-button"' . $tag_atts . '>' . __( 'Follow', 'selftitled' ) . ' ' . $username . '</a>' . "\n";
