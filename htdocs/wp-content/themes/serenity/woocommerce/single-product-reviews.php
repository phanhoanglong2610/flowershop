...
<?php if ( comments_open() ) : ?><div id="reviews"><?php
	...
	if ( $count > 0 ) :
		...
		echo '<h5>'.sprintf( _n('%s review for %s', '%s reviews for %s', $count, 'woocommerce'), '<span itemprop="ratingCount" class="count">'.$count.'</span>', wptexturize($post->post_title) ).'</h5>';
	...
	if ( have_comments() ) :
		...
		echo '<p class="add_review"><a href="#review_form" class="btn inline show_review_form button">'.__('Add Review', 'woocommerce').'</a></p>';
